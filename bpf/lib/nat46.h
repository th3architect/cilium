 ...
 * d4 = d6[96 .. 127]
 */
static inline int ipv6_to_ipv4(struct __sk_buff *skb, int nh_off, __u32 saddr)
{
	struct ipv6hdr v6;
	struct iphdr v4 = {};
	int csum_off;
...
static inline int isMappedIPv6Addr(union v6addr *addr)
{
	if (addr->p1 == 0 && addr->p2 == 0 && addr->p3 == 0xFFFF0000)
		return 1;
	else
		return 0;
}
